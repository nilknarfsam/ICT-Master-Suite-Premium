...
### [2026-05-06] — [Pre-Fase 4.3] — Refinamento operacional pos-4.2 (Finder Logs prioritario)
- Objetivo: corrigir, sem mudar contratos, problemas observados em smoke visual real da build pos-4.2: janela cortada, dashboard como aba primaria, painel TRI ocupando muito espaco vertical, lista de logs sem ordenacao por data, textos legados — preparando o workspace para a Subfase 4.3.
- Prompt aplicado/resumo: em `src/app_desktop/ui_main.py`: (1) reordenadas as abas para Finder, Wiki, Historico, Admin, Config, Dashboard (lazy, ultimo); (2) adicionado import `QGuiApplication` e geometria responsiva via `availableGeometry` com fallback explicito; (3) painel "Detalhamento de Defeitos (TRI)" ocultado visualmente em `setup_finder` e `on_file_loaded` (parser/populate intactos); (4) `txt_observacao` migrado de `setFixedHeight(60)` para `setMinimumHeight(90)` e adicionados `setStretchFactor` para `text_raw` (3) e `txt_historico_chat` (2); (5) splitter `[300, 800]` -> `[320, 980]`; (6) `popular_lista` agora ordena por `os.path.getmtime` desc com fallback `0`, exibe label `"YYYY-MM-DD HH:MM - nome"` e armazena nome canonico em `Qt.UserRole`; (7) `carregar_arquivo` passa a usar `item.data(Qt.UserRole) or item.text()`; (8) textos atualizados ("Logs encontrados:", "Selecione um log para visualizar.", "Visualizacao do Log:"). Documentos atualizados/criados conforme abaixo.
- Arquivos principais: `src/app_desktop/ui_main.py`, `docs/PHASE_4_3_SEARCH_EXPERIENCE_PREP.md` (novo), `docs/PHASE_4_COMPLETION_EXECUTION_PLAN.md` (status), `CHANGELOG.md`, `docs/PROJECT_EVOLUTION_LOG.md`.
- Commit: pendente (a preencher apos commit `refactor(ui): prioritize finder logs workspace`).
- Testes/validacao: `python -m pytest tests/app_desktop tests/application tests/core --tb=short` (escopo conservador, evita falha pre-existente em `tests/parsers/test_tri_parser.py::test_parse_tri_txt_pass_serial_modelo`); `scripts\\dev_check.bat` registrando a falha TRI como debito tecnico mapeado caso ocorra.
- Observacoes: rodada operacional **nao** renumera subfases formais; `src/app_desktop/threads.py`, application services, core, parsers, esquema SQLite e entregas das fases 4.1/4.2 permanecem intocados; o painel TRI volta a ser exibido naturalmente assim que a Subfase 4.4 (Log Viewer Premium) entregar uma `LogDetailPanel`; falha pre-existente do parser TRI segue como debito mapeado a ser tratado fora desta rodada.
- Proximo passo: executar a Subfase 4.3 completa (Search Experience Premium) com `widgets/search/`, `viewmodels/search/finder_viewmodel.py`, extracao de `FinderPage`, integracao em `ui_main.py` e suite de testes em `tests/app_desktop/finder/`.
